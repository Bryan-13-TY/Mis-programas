...
List *list_create(void) {
    List *list = malloc(sizeof(List)); // creamos la lista
...
void insert_end(List *list, int num) {
    if (!list) { // inicialización de la lista
    ...
    if (!new) { // fallo al reservar memoria
...
static bool validate_position(List *list, int pos) {
    if (pos > list->count + 1) {
...
void insert_in_position(List *list, int num, int pos) {
    if (!validate_position(list, pos)) { // posición no valida
        return;
    }
    ...
    Node *before_new = list->head; // posición entes de la indicada
...
void free_list(List *list) {
    ...
    printf("\n==> La lista se ha vaciado/destruida correctamente");
}
