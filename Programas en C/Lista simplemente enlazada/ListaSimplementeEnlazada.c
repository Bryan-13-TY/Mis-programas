/**
 * @file ListaSimplementeEnlazada.c
 * @brief Crea una lista de números simplemente enlazada, con la que se pueden hacer diferentes operaciones.
 * 
 * Este archivo contiene las funciones necesarias para la creación de una lista simplemente enlazada de números, así
...
void LiberarNum(int pos, int *counter) {
    ...
    before = move; // Apuntamos al número anterior
    delete = move -> sgt; // Apuntamos al número que queremos eliminar
    after = delete -> sgt; // Apuntamos al número siguiente
    free(delete); // Liberamos el número

    before -> sgt = after; // Unimos las dos listas
...
void LiberarLista() {
    ...
    while (delete != NULL) {
        move = delete -> sgt; // Apuntamos al nodo siguiente
        free(delete); // Liberamos el nodo actual
        delete = move; // Actualizamos
    }
