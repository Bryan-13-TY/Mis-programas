import socket, struct, wave, json
...
def recibir_audio_gbn(sock: socket.socket, cliente_addr: tuple, info: dict, timeout=5.0):
    """Recibe un archivo de audio enviado con Go-Back-N y lo guarda como .wav"""

    nombre = info["nombre"]
    tamano = info["tamano"]
    total_paquetes = info["total_paquetes"]
    ...
    while True:
        ...
        if paquete == b"FIN":
            ...
            sock.settimeout(None) # Desactiva el timeout
            break
...
def main():
    ...
    print(f">> Servidor Go-Back-N JSON escuchando en {SERVER_ADDR}")
    ...
    while True:
        ...
        try:
            ...
            sock.sendto(json.dumps({"error": "Formato no válido"}).encode(), addr)
            continue

        if mensaje.get("tipo") == "AUDIOINFO":
            sock.sendto(json.dumps({"tipo": "READY"}).encode(), addr)
            recibir_audio_gbn(sock, addr, mensaje)
        else:
            sock.sendto(json.dumps({"error": "Comando desconocido"}).encode(), addr)
