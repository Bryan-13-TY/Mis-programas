...
sock.bind(SERVER_ADDR) # se vincula el socket al puerto del servidor
    ...
    while True:
        ...
        if tipo == "inicio":
            if usuario not in usuarios[sala]: # si el usuario no esta en la sala anteriormente
                ...
                aviso = {"tipo": "aviso",
                         "sala": sala,
                         "content": f">> {utl.GREEN}[+]{utl.RESET}{utl.BLUE}[{usuario}]{utl.RESET} se ha unido a la sala"}
        ...
        # ---- 4. mensaje ----
            ...
            # ---- 4.2.- mensaje privado ----
                ...
                if dest in usuarios[sala]: # si el destinatario esta en la sala
                    ...
                    error = {"tipo": "aviso",
                             "sala": sala,
                             "content": f">> {utl.RED}[system]{utl.RESET} Usuario '{dest}' no está conectado"}
        ...
        # ---- 5. un usuario abandona la sala ----
            ...
            if usuario in usuarios[sala]: # si el usuario esta actualmente en la sala
                ...
                aviso = {"tipo": "aviso",
                         "sala": sala,
                         "content": f">> {utl.RED}[-]{utl.RESET}{utl.BLUE}[{usuario}]{utl.RESET} ha abandonado la sala"}
        ...
        # ---- 6. audio ----
            ...
            if not mensaje.get("privado", False):
                aviso = {"tipo": "aviso",
                         "sala": sala,
                         "content": f"{utl.GREEN}[🎙️][{usuario}]{utl.RESET} ha enviado el audio '{mensaje.get("nombre")}'"}
