...
class Sala:
    def __init__(self, nombre_sala, area_mensajes, entrada_mensaje) -> None:
        self.nombre_sala = nombre_sala
        self.area_mensajes = area_mensajes
        self.entrada_mensaje = entrada_mensaje

    def cargar_usuarios(self) -> list[str]:
        """
    ...
    def enviar_mensaje(self) -> None:
        ...
        if (texto.strip()):
            self.area_mensajes.insert(tk.END, f"Usuario: {texto}\n")
            self.entrada_mensaje.delete(0, tk.END)

    def crear_sala(self) -> None:
        sala = tk.Tk()
        sala.title(self.nombre_sala)
        ...
        usuarios = Sala.cargar_usuarios(self)
        ...
        enviar_mensaje = Sala.enviar_mensaje(self)
...
def main():
    sala1 = Sala.crear_sala("Lalito XDE", area_mensajes, entrada_mensaje)
