...
def listarArticulos_S(rutaArticulos: str, conexion : socket.socket) -> None:
    """
    Parameters
    ----------
    rutaArticulos : str
    ...
    """
    articulos = guardarArticulos(rutaArticulos) # Obtenemos el diccionario con los artículos
...
rutaJSON_A = obtenerRuta() # Obetenos la ruta del archivo JSON, donde se guardan los artículos
    ...
    while (True): # Bucle para mantener la conexión abierta
        try: # Intentamos verificar si el cliente sigue conectado
            ...
            if (solicitud["accion"] == "LISTAR_ARTICULOS"): # Listar artículos
                listarArticulos_S(rutaJSON_A, conn)
            elif (solicitud["accion"] == "BUSCAR_ARTICULOS"): # Buscar artículos
                buscarArticulo_S(rutaJSON_A, solicitud["buscar"], conn)
            elif (solicitud["accion"] == "CARRITO"): # Agregar carrito
                print(solicitud["buscar"])
            else:
